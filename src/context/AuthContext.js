...
import AttendanceService from '../services/AttendanceService'; // <-- added
...
export const AuthProvider = ({ children }) => {
  ...
  // --- LOGOUT with cleanup of user‑specific attendance data ---
  const logout = async () => {
    if (user?.id) {
      await AttendanceService.clearUserAttendanceData(user.id);
    }
    try {
